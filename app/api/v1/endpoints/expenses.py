...
async def get_expense_summary(
    ...
    **Returns**:
    ...
    pending_expenses, pending_amount = await get_stats(
        [Expense.status == ExpenseStatus.PENDING]
    )

    # Get approved expenses count and amount
    approved_expenses, approved_amount = await get_stats(
        [Expense.status == ExpenseStatus.APPROVED]
    )

    # Get rejected expenses count and amount
    rejected_expenses, rejected_amount = await get_stats(
        [Expense.status == ExpenseStatus.REJECTED]
    )

    # Get reimbursed expenses count and amount
    reimbursed_expenses, reimbursed_amount = await get_stats(
    ...
    if current_user.role == UserRole.EMPLOYEE:
        ...
        if not employee:
            return ExpenseListResponse(
                expenses=[], total=0, total_amount=Decimal("0.00")
            )
...
async def approve_expense(
    ...
    **Workflow**: Changes status from "pending" to "approved".
    ...
    logger.info("Expense %s approved by user %s", expense.id, current_user.id)
...
async def reject_expense(
    ...
    **Workflow**: Changes status from "pending" to "rejected".
    ...
    logger.info("Expense %s rejected by user %s", expense.id, current_user.id)
...
async def reimburse_expense(
    ...
    **Workflow**: Changes status from "approved" to "reimbursed".
    ...
    logger.info("Expense %s reimbursed by user %s", expense.id, _current_user.id)
