...
async def create_user(
    ...
    **Response**: Created user details
    ...
    stmt = select(User).where(
        User.email == user_data.email, User.company_id == current_user.company_id
    )
    ...
    logger.info("User %s created by admin %s", new_user.id, current_user.id)
...
async def deactivate_user(
    ...
    **Company Isolation**: Can only deactivate users from the same company.
    ...
    logger.info("User %s deactivated by admin %s", user.id, current_user.id)
