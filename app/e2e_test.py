...
async def main():
    ...
    async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
        ...
        if not company_data["company"]["slug"].startswith(expected_slug):
             logger.warning(f"⚠️ Warning: Slug mismatch or suffixed? Got: {company_data['company']['slug']}")
        company_slug = company_data["company"]["slug"]
        logger.info(f"✅ Company Created: {company_data['company']['name']} (ID: {company_data['company']['id']}, Slug: {company_slug})")
        ...
        logger.info(f"✅ Employee Record Created: {employee_record['name']} (ID: {employee_id})")
        ...
        logger.info(f"✅ Expense Submitted: {expense_record['title']} (ID: {expense_id})")
        ...
        if approved_expense["status"] != "approved":
            logger.error(f"❌ Expense status not approved: {approved_expense['status']}")
            return
        ...
        if res.status_code == 403:
             logger.info("✅ RBAC Check Passed: Employee cannot create assets (403 Forbidden)")
        else:
        ...
        if res.status_code == 403:
             logger.info("✅ RBAC Check Passed: Employee cannot approve expenses (403 Forbidden)")
        else:
             logger.error(f"❌ RBAC Check Failed: Expected 403, got {res.status_code}")
        ...
        if res.status_code == 404:
             logger.info("✅ Tenant Isolation Passed: Company B cannot see Company A's employee (404 Not Found)")
        else:
             logger.error(f"❌ Tenant Isolation Failed: Expected 404, got {res.status_code}")
