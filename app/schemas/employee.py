...
class EmployeeBase(BaseModel):
    ...
    employee_id: Optional[str] = Field(
        None, max_length=50, description="Optional employee ID/code"
    )
...
class EmployeeCreate(EmployeeBase):
    ...
    user_id: Optional[UUID] = Field(
        None,
        description="Optional user account ID to link employee to. Employees can exist without user accounts.",
    )
...
class EmployeeUpdate(BaseModel):
    ...
    phone: Optional[str] = Field(None, max_length=20)
    user_id: Optional[UUID] = Field(
        None, description="Optional user account ID to link employee to"
    )
