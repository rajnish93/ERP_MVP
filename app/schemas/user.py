...
class UserInvite(UserBase):
    ...
    department: Optional[str] = Field(
        ...
        min_length=1,
        max_length=100,
        description="Department name (used if creating new employee record)",
    )
    job_role: Optional[str] = Field(
        None,
        min_length=1,
        max_length=100,
        description="Job role/title (used if creating new employee record)",
    )
    joining_date: Optional[datetime] = Field(
        None, description="Employee joining date (used if creating new employee record)"
    )
