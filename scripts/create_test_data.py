...
async def cleanup_existing_data(db: AsyncSession):
    ...
    try:
        ...
        if asset_count > 0 or employee_count > 0 or user_count > 0 or company_count > 0 or expense_count > 0:
...
async def create_test_data():
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            for company_idx, company_data in enumerate(created_companies):
                company = company_data["company"]
                company_assets = [a for a in all_assets_list if a.company_id == company.id]
            ...
            expense_categories = ["Travel", "Office Supplies", "Software License", "Meals", "Training", "Hardware"]
            ...
            for company_idx, company_data in enumerate(created_companies):
                ...
                for _ in range(num_expenses):
                    ...
                    date = fake.date_time_between(start_date="-3m", end_date="now", tzinfo=timezone.utc)
                    ...
                    if status in [ExpenseStatus.APPROVED, ExpenseStatus.REJECTED, ExpenseStatus.REIMBURSED]:
                        ...
                        approved_at = fake.date_time_between(start_date=date, end_date="now", tzinfo=timezone.utc)
                    ...
                    print(f"  ✅ {company.name}: {expense.title} (${amount}) - {status.value}")
            ...
            result = await db.execute(select(func.count(Employee.id)).where(Employee.user_id.isnot(None)))
            ...
            result = await db.execute(select(func.count(Company.id)).where(Company.plan_type == PlanType.FREE))
            print(f"  - FREE plan: {result.scalar()}")
            
            result = await db.execute(select(func.count(Company.id)).where(Company.plan_type == PlanType.PRO))
            print(f"  - PRO plan: {result.scalar()}")
            
            result = await db.execute(select(func.count(Company.id)).where(Company.plan_type == PlanType.ENTERPRISE))
            ...
            result = await db.execute(select(func.count(User.id)).where(User.role == UserRole.HR))
            print(f"  - HR: {result.scalar()}")
            
            result = await db.execute(select(func.count(User.id)).where(User.role == UserRole.EMPLOYEE))
            ...
            result = await db.execute(select(func.count(Asset.id)).where(Asset.assigned_to.isnot(None)))
            ...
            for status in expense_statuses:
                result = await db.execute(select(func.count(Expense.id)).where(Expense.status == status))
                count = result.scalar()
            ...
            for asset_type in asset_types:
                result = await db.execute(select(func.count(Asset.id)).where(Asset.asset_type == asset_type))
                count = result.scalar()
            ...
            for idx in range(len(created_companies)):
                ...
                stmt = select(Employee).where(Employee.company_id == company.id, Employee.user_id.isnot(None)).limit(3)
                result = await db.execute(stmt)
